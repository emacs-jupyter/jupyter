...
There is interference between =ob-async= and Jupyter source blocks with the
=:async yes= header argument which causes Jupyter source blocks to not work
correctly. To get around this you can set
[[https://github.com/astahlman/ob-async#ob-async-no-async-languages-alist][ob-async-no-async-languages-alist]] to a value that excludes Jupyter source
blocks from being handled by =ob-aync=.
