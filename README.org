...
notebook server located at =http://localhost:8888=. If =NAME= is a kernel ID
corresponding to an existing kernel on a server, e.g.
=/jpy::161b2318-180c-497a-b4bf-de76176061d9=, then a connection to an existing
kernel with the corresponding ID will be made. Otherwise, a new kernel will be
launched on the server and =NAME= will be used as an identifier for the
session.

Note, the =:kernel= header argument must match the name of the kernelspec of
the kernel being connected to when =NAME= is a kernel ID of an existing kernel.
