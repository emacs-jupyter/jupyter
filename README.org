...
revisited by calling =help-go-back= or =help-go-forward= while in the =*Help*=
buffer. Making completion requests to a kernel is done through the
=completion-at-point= interface. If the kernel asks for input from the user, a
prompt is displayed in the minibuffer. These are just a few of the ways that
this package integrates with Emacs' built-in features.
