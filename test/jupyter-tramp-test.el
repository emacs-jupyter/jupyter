...
(ert-deftest jupyter-tramp-file-name-p ()
  ...
  (should (equal (jupyter-tramp-file-name-p "/jupy::foobar") "jupy"))
  (should (equal (jupyter-tramp-file-name-p "/jupy::/foobar") "jupy"))
  (should (equal (jupyter-tramp-file-name-p "/jupys::/foobar") "jupys")))
...
(ert-deftest jupyter-tramp-file-directory-p ()
  ...
  (jupyter-test-at-temporary-directory
    (let* ((tfile (make-temp-file "file-directory-p"))
           ...
           (jupyfile (expand-file-name (file-name-nondirectory tfile) "/jupy::/"))
           (jupydir (expand-file-name (file-name-nondirectory tdir) "/jupy::/")))
      (unwind-protect
          (progn
            (should (file-exists-p jupyfile))
            (should (file-exists-p jupydir))
            (should-not (file-directory-p jupyfile))
            (should (file-directory-p jupydir)))
...
(ert-deftest jupyter-tramp-file-writable-p ()
  ...
  (jupyter-test-at-temporary-directory
    (let* ((tname (make-temp-name "file-writable-p"))
           (jupyfile (expand-file-name tname "/jupy::/")))
      ...
      (unwind-protect
          (let ((contents (concat "αβ" (jupyter-new-uuid) "λ")))
            (with-temp-file tfile
              ...
              (insert contents))
            (let ((lfile (file-local-copy jupyfile)))
              (unwind-protect
                  (with-temp-buffer
...
(ert-deftest jupyter-tramp-rename-file ()
  ...
  (jupyter-test-at-temporary-directory
    (let* ((tfile (make-temp-file "rename-file"))
           ...
           (jupyfile (expand-file-name (file-name-nondirectory tfile) "/jupy::/"))
           (jupynewname (expand-file-name tnewname "/jupy::/")))
      (ert-info ("Remote to same remote")
        ...
        (unwind-protect
            (let ((contents (jupyter-new-uuid)))
              (with-temp-file tfile
                (insert contents))
              (rename-file jupyfile jupynewname)
      ...
      (ert-info ("Local to remote")
        (unwind-protect
            (let ((contents (jupyter-new-uuid)))
              ...
              (should-not (file-exists-p jupyfile))
              (with-temp-file tfile
                (insert contents))
              (should (file-exists-p tfile))
              (rename-file tfile jupynewname)
              (should-not (file-exists-p tfile))
              (should (file-exists-p tnewname)))
          (ignore-errors (delete-file tnewname))
          (ignore-errors (delete-file tfile)))))))
    ...
    (cl-macrolet
      ...
      (ert-info ("Local to remote")
        (let ((tf1 (make-temp-name "copy-file"))
              (jupy1 (expand-file-name (make-temp-name "copy-file") "/jupy::/"))
              (c1 (jupyter-new-uuid)))
          (check-copy tf1 jupy1 c1)))
      (ert-info ("Remote to local")
        (let ((tf1 (make-temp-name "copy-file"))
              (jupy1 (expand-file-name (make-temp-name "copy-file") "/jupy::/"))
              (c1 (jupyter-new-uuid)))
          (check-copy jupy1 tf1 c1)))
      (ert-info ("Remote to remote")
        (let ((jupy1 (expand-file-name (make-temp-name "copy-file") "/jupy::/"))
              (jupy2 (expand-file-name (make-temp-name "copy-file") "/jupy::/"))
              (c1 (jupyter-new-uuid)))
          (check-copy jupy1 jupy2 c1))))))
      ...
      (unwind-protect
          (progn
            (ert-info ("Error when attempting to delete a directory")
              (should-error (delete-file jupydir)))
            (ert-info ("Delete a file")
              (delete-file jupyfile)
              (should-not (file-exists-p tfile))
      ...
      (unwind-protect
          (progn
            (ert-info ("Error when attempting to delete a file")
              (should-error (delete-directory jupyfile)))
            (ert-info ("Delete a directory")
              (let ((jupyfile2 (expand-file-name "foobar" jupydir)))
                (write-region "xxx" nil jupyfile2)
                (unwind-protect
                    (progn
                      (ert-info ("Error when directory contains files")
                        (should-error (delete-directory jupydir)))
                      (ert-info ("Unless recursive is specifed")
                        (delete-directory jupydir t)
                        (should-not (file-exists-p jupyfile2))
                        (should-not (file-directory-p tdir))))
                  (when (file-exists-p jupyfile2)
                    (delete-file jupyfile2))))
              (should-not (file-exists-p tdir))
              (ert-info ("Ensure cache is cleared")
                (should-not (file-exists-p jupydir)))))
        (when (file-exists-p tfile)
          (delete-file tfile))
        (when (file-exists-p tdir)
...
(ert-deftest jupyter-tramp-file-attributes ()
  ...
  (jupyter-test-at-temporary-directory
    (let* ((file (make-temp-file "file-attributes"))
           (jupyfile (expand-file-name
                     (file-name-nondirectory file) "/jupy::/")))
      ...
      (unwind-protect
          (let ((attrs (file-attributes file 'string))
                (jupyattrs (file-attributes jupyfile 'string)))
            ...
            (dolist (s '(2 3))
              ...
              (setf (nth s (file-attribute-modification-time jupyattrs)) 0)
              (setf (nth s (file-attribute-status-change-time jupyattrs)) 0))
            (should (equal (nth 0 attrs) (nth 0 jupyattrs)))
            (dolist (item '(file-attribute-modification-time
              ...
              (should (equal (funcall item attrs)
                             (funcall item jupyattrs)))))
        (delete-file file)))))

(ert-deftest jupyter-tramp-expand-file-name ()
  :tags '(tramp server)
  (should (equal "/foo" (tramp-drop-volume-letter (expand-file-name "/foo" "/jupy:h:/foo"))))
  (should (equal "~/foo" (abbreviate-file-name (expand-file-name "~/foo" "/jupy:h:/foo"))))
  (should (equal "/jupy:h:/foo/bar" (expand-file-name "bar" "/jupy:h:/foo")))
  (should (equal "/jupy:h:/foo/bar" (expand-file-name "bar" "/jupy:h:/foo/")))
  (should (equal "/jupy:h:/foo/bar" (expand-file-name "/jupy:h:/foo/bar")))
  (should (equal "/jupy:h:/foo/bar" (expand-file-name "/jupy:h:foo/bar")))
  (let ((default-directory "/jupy:h:/"))
    (should (equal "/jupy:h:/foo" (expand-file-name "foo"))))
      ...
      (unwind-protect
          (cl-macrolet ((file-contents
                         () `(with-temp-buffer
                               ...
                               (buffer-string))))
            (should-error (write-region "foo" nil jupyfile nil nil nil 'excl))
            (ert-info ("Basic write")
              (write-region "foo" nil jupyfile)
              (should (equal (file-contents) "foo"))
              (write-region "foλo" nil jupyfile)
              (should (equal (encode-coding-string (file-contents) 'utf-8)
                             (encode-coding-string "foλo" 'utf-8)))
              (with-temp-buffer
                (insert "foo")
                (write-region nil nil jupyfile)
                (should (buffer-modified-p))
                (should (equal (file-contents) "foo"))
                (insert "bar")
                (write-region nil "" jupyfile)
                (should (buffer-modified-p))
                (should (equal (file-contents) "foobar"))
                (should-error (write-region 1 nil jupyfile))
                (should-error (write-region (list 1) 1 jupyfile))
                (write-region 2 4 jupyfile)
                (should (buffer-modified-p))
                (should (equal (file-contents) "oo"))))
            (ert-info ("Base64 encode binary")
              (let ((coding-system-for-write 'binary))
                (write-region "\0\1\2\3\4\5\6" nil jupyfile)
                (should (equal (file-contents) "\0\1\2\3\4\5\6"))))
            (ert-info ("Append")
              (write-region "x" nil jupyfile)
              (should (equal (file-contents) "x"))
              (write-region "y" nil jupyfile t)
              (should (equal (file-contents) "xy"))
              (write-region "z" nil jupyfile t)
              (should (equal (file-contents) "xyz"))
              (write-region "a" nil jupyfile 1)
              (should (equal (file-contents) "xaz"))
              (write-region "β" nil jupyfile 6)
              (should (equal (encode-coding-string (file-contents) 'utf-8)
                             (encode-coding-string "xaz\0\0\0β" 'utf-8))))
            (ert-info ("File visiting")
              (with-temp-buffer
                (insert "foo")
                (write-region nil nil jupyfile nil t)
                (should-not (buffer-modified-p))
                (should (equal jupyfile (buffer-file-name)))
                (insert "bar")
                (write-region nil nil jupyfile nil "foo")
                (should-not (buffer-modified-p))
